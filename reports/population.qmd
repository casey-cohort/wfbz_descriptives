...
```{r data_check}
expected_parquet <- c('race', 'poverty_lt_100', 'vehicle_avail', 'housing_cost_burden')
missing_parquet <- expected_parquet[
  !file.exists(here('data/processed/nhgis', paste0(expected_parquet, '.parquet')))
]
...
census_sf <- reduce(
  list(
    race %>%
      mutate(census_year = 10 * floor(start_year / 10)) %>%
      group_by(GEOID, census_year) %>%
      nest(.key = 'race'),
    poverty %>%
      mutate(census_year = 10 * floor(start_year / 10)) %>%
      group_by(GEOID, census_year) %>%
    ...
    vehicle_avail %>%
      mutate(census_year = 10 * floor(start_year / 10)) %>%
      group_by(GEOID, census_year) %>%
      nest(.key = 'vehicle_avail'),
    housing_cost %>%
      mutate(census_year = 10 * floor(start_year / 10)) %>%
      group_by(GEOID, census_year) %>%
      nest(.key = 'housing_cost'),
    tract_sf
  ),
  full_join,
...
census_data_by_wf <- st_join(
  ...
  filter(between(
    wildfire_year,
    `vehicle_avail|start_year`,
    `vehicle_avail|end_year`
  )) %>%
  ...
  filter(between(
    wildfire_year,
    `housing_cost|start_year`,
    `housing_cost|end_year`
  )) %>%
