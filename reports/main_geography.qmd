...
wfbz %>%
  ...
  geom_boxplot(
    aes(
      x = as.Date('2099-01-01') + yday(wildfire_ignition_date_max),
      group = wildfire_year,
      y = wildfire_year,
      fill = usfs_region
    ),
  ...
  geom_boxplot(
    ...
    outlier.shape = NA,
    coef = 0, # hide all but the box body on top layer
    orientation = 'y',
    outliers = FALSE
  ) +
  geom_point(
    # replace hidden median bar with a dot
    aes(
      x = as.Date('2099-01-01') + yday(wildfire_ignition_date_max),
      y = wildfire_year
    ),
  ...
  scale_x_date(
    date_breaks = "1 month",
    date_labels = "%b",
    limits = as.Date(c('2099-01-01', '2099-12-31')),
    expand = c(0, 0)
  ) +
...
ggsave(
  here('figures/fire_season_timing.pdf'),
  last_plot(),
  width = 10,
  height = 12
)
...
# Order the factor by ascending plot_order so the legend matches the column
# plots (plotme orders regions the same way).
region_order <- REGION_ORDER %>% arrange(plot_order) %>% pull(usfs_region)
...
ggsave(here('figures/regions_map.pdf'), last_plot(), width = 8, height = 6)
```
