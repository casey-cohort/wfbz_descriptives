...
p_rank <- ggplot() +
  geom_vline(
    xintercept = mean(by_state$`FEMA Declaration Percent`),
    color = 'grey25',
    linetype = 'dashed'
  ) +
  annotate(
    geom = 'text',
    x = mean(by_state$`FEMA Declaration Percent`),
    y = 'Ohio',
    color = 'grey25',
    label = 'Mean',
    angle = -90,
  ...
  scale_x_continuous(
    ...
    name = 'Percent of events declared a disaster',
    expand = c(0, 0, 0, .125)
  ) +
  ...
  scale_y_discrete(name = '', limits = rev, expand = c(.025, .025)) +
...
p_rank
ggsave(
  here('figures/fema_decs_ranked_by_state.pdf'),
  last_plot(),
  width = 9,
  height = 9
)
...
header_natural <- wrap_elements(grid::textGrob(
  "Natural scale",
...
ggsave(
  here('figures/fema_decs_scatters_INTERNAL.pdf'),
  p,
  width = 10,
  height = 40
)
...
p_map
ggsave(
  here('figures/maps_decs_per_event.pdf'),
  last_plot(),
  width = 8,
  height = 5
)
...
ggsave(
  here('figures/fema_rank_map.pdf'),
  last_plot(),
  width = 10,
  height = 14
)
...
#  coord_sf(expand = FALSE)
#ggsave(
#  here('figures/maps_decs_per_event_log.pdf'),
#  last_plot(),
#  width = 8,
#  height = 5
#)
