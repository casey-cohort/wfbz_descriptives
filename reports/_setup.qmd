...
# Bind the ramp to regions by ascending `plot_order`, so it reads in order along
# the stack: Hawaii (plot_order 1) takes the dark end, California (10) the light
# end. Flip to arrange(desc(plot_order)) to reverse. Do NOT bind with
# sort(usfs_region) -- alphabetical order has no relation to plot_order and
# scrambles the ramp.
region_colors <- setNames(
  wfbz_ramp,
  REGION_ORDER %>% arrange(plot_order) %>% pull(usfs_region)
)
