...
# Palette
# 10-color ramp, dark -> light. Hardcoded rather than sampled so renders are
# reproducible and match _pop_setup.qmd exactly.
wfbz_ramp <- c(
  "#4A3A3B", "#6C3D38", "#8F4036", "#A64E4C", "#B8606A",
  "#CB7D82", "#DEA393", "#ECC5A8", "#EEDAC6", "#F0F0E4"
)
# Bind the ramp to regions by descending `plot_order`, so it reads in order along
# the stack: Hawaii (plot_order 1) takes the light end, California (10) the dark
# end. Flip to arrange(plot_order) to reverse. Do NOT bind with
# sort(usfs_region) -- alphabetical order has no relation to plot_order and
# scrambles the ramp.
region_colors <- setNames(
  wfbz_ramp,
  REGION_ORDER %>% arrange(desc(plot_order)) %>% pull(usfs_region)
)
...
plotme <- function(
  ...
  if (!stack) {
    ...
    region_label <- paste0(
      ...
      ' (',
      trim(format(100 * region_summary$pcttot, digits = 1)),
      '%)'
    )
  } else {
    region_label <- paste0(
  ...
  # Re-key the palette onto the decorated labels. Index by region name so each
  # region keeps its own color, then rename positionally -- `region_order` and
  # `region_label` are parallel. (Sorting the labels instead would re-scramble
  # the ramp, and breaks outright when `dat` is missing a region.)
  region_colors <- setNames(region_colors[region_order], region_label)
