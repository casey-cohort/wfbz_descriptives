...
wfbz <- read_sf("data/raw/wfbz_disasters_2000-2025.geojson") %>% filter(!st_is_empty(geometry))
wfbz <- wfbz %>% filter(!str_detect(wildfire_complex_names, '(FLOOD$|FLOOD 2013$)')) # remove some colorado floods that snuck in
...
regions$usfs_region_pop <- aggregate(st_centroid(counties_pop), regions, FUN = sum)$population # centroid more or less ensures counties only end up in one region
...
wfbz_region_area <- as.numeric(st_area(regions)[st_nearest_feature(wfbz, regions, pairwise = TRUE)]/1000/1000)
wfbz_region_pop <- regions$usfs_region_pop[st_nearest_feature(wfbz, regions, pairwise = TRUE)]
...
wfbz_crit <- wfbz %>%
  ...
  summarize(
  	...
  	`Fatalities per 100K` = 100000 * sum(wildfire_max_civil_fatalities/usfs_region_pop, na.rm = TRUE),
  	...
  	`FEMA Declaration` = sum(wildfire_fema_dec, na.rm = TRUE),
   	...
   	`Burned Area Percent` = 100 * sum(wildfire_area/usfs_region_area, na.rm = TRUE),
...
plotme <- function(dat, var, rel = FALSE, unit = NA, stack = TRUE){
...
wfbz %>%
  ...
  mutate(
	...
	bind_rows(`All Fires` = ., `Community-Impacting Fires` = filter(.,wildfire_community_intersect), .id = 'agg') %>%
  bind_rows(., mutate(., usfs_region = 'Nationwide', usfs_region_area = sum(regions$usfs_region_area), usfs_region_pop = sum(as.numeric(st_area(regions))/1000/1000))) %>%
  group_by(agg) %>%
	mutate(N = n()/2) %>%
  group_by(agg, usfs_region, N) %>%
  summarize(
    `Count` = n(),
    `WUI Interface Fires` = sum(str_detect(wildfire_wui, 'interface'), na.rm = TRUE),
    `WUI Intermix Fires` = sum(str_detect(wildfire_wui, 'intermix'), na.rm = TRUE),
    `WUI Any Fires` = sum(str_detect(wildfire_wui, 'interface') & str_detect(wildfire_wui, 'intermix'), na.rm = TRUE),
    `Missing WUI` = 0,
	...
	mutate(
		value = if_else(
			name %in% c('Fatalities per 100K', 'Burned Area Percent'),
			format(value, digits = 1),
			glue('{round(value)} ({format(100 * value / N, digits = 1, nsmall = 1)}%)')
		)
	) %>%
	select(-N) %>%
...
```{r, fig.height = 8, fig.width = 12}
plotme(wfbz_crit, 'WUI-Interface', rel = FALSE) + ggtitle('Community-Impacting\nDisasters') +
	plotme(wfbz_crit_all, 'WUI-Interface', rel = FALSE) + ggtitle('All Disasters') +
	plot_spacer() + plot_spacer() +
	plotme(wfbz_crit, 'WUI-Interface', rel = TRUE) +
	plotme(wfbz_crit_all, 'WUI-Interface', rel = TRUE) +
	plot_layout(ncol = 2, heights = c(5,1,5))
```
...
```{r, fig.height = 8, fig.width = 12}
plotme(wfbz_crit, 'WUI-Intermix', rel = FALSE) + ggtitle('Community-Impacting\nDisasters') +
	plotme(wfbz_crit_all, 'WUI-Intermix', rel = FALSE) + ggtitle('All Disasters') +
	plot_spacer() + plot_spacer() +
	plotme(wfbz_crit, 'WUI-Intermix', rel = TRUE) +
	plotme(wfbz_crit_all, 'WUI-Intermix', rel = TRUE) +
	plot_layout(ncol = 2, heights = c(5,1,5))
```
...
plot_spacer() +
	(plotme(wfbz_crit, 'WUI-Interface Percent', stack = FALSE) + ggtitle('Community-Impacting\nDisasters')) +
	(plotme(wfbz_crit_all, 'WUI-Interface Percent', stack = FALSE) + ggtitle('All Disasters') + ylab('')) +
	plot_layout(ncol = 4) +
	plot_spacer()
```
...
```{r, fig.height = 16, fig.width = 12}
plot_spacer() +
	(plotme(wfbz_crit, 'WUI-Intermix Percent', stack = FALSE) + ggtitle('Community-Impacting\nDisasters')) +
	(plotme(wfbz_crit_all, 'WUI-Intermix Percent', stack = FALSE) + ggtitle('All Disasters') + ylab('')) +
	plot_spacer() +
	plot_layout(ncol = 4)
```
