...
wfbz %>%
  st_drop_geometry() %>%
  mutate(usfs_region = wfbz_region) %>%
  bind_rows(`All` = ., `Community-Impacting` = filter(.,wildfire_community_intersect), .id = 'agg') %>%
  group_by(agg) %>% mutate(N = n()) %>%
  bind_rows(., mutate(., usfs_region = 'Nationwide')) %>%
  group_by(agg, usfs_region, N) %>%
  summarize(
    n = n(),
    wui_interface = sum(str_detect(wildfire_wui, 'interface'), na.rm = TRUE),
    wui_intermix = sum(str_detect(wildfire_wui, 'intermix'), na.rm = TRUE),
    wui_interface_intermix = sum(str_detect(wildfire_wui, 'interface') & str_detect(wildfire_wui, 'intermix'), na.rm = TRUE),
    wui_missing = 0,
    ignition_date_missing = sum(is.na(wildfire_ignition_date)),
    containment_date_missing = sum(is.na(wildfire_containment_date_max)),
    civil_fatalities_missing = sum(is.na(wildfire_civil_fatalities)),
    civil_evacs_missing = sum(is.na(wildfire_civil_evacuation)),
    struct_destroyed_missing = sum(is.na(wildfire_struct_destroyed))
  ) %>%
  pivot_wider(names_from = agg, values_from = c(-agg, -usfs_region, -N)) %>%
  gt()
```
