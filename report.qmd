...
wfbz %>%
  ...
  bind_rows(
    `All Fires` = .,
    `Community-Impacting Fires` = filter(., wildfire_community_intersect),
    .id = 'agg'
  ) %>%
  bind_rows(
    .,
    mutate(
      ...
      usfs_region_area = sum(regions$usfs_region_area),
      usfs_region_pop = sum(as.numeric(st_area(regions)) / 1000 / 1000)
    )
  ) %>%
  group_by(agg) %>%
  mutate(N = n() / 2) %>%
  ...
  mutate(
    value = if_else(
      ...
      glue(
        '{round(value)} ({format(100 * value / N, digits = 1, nsmall = 1)}%)'
      )
    )
  ) %>%
  select(-N) %>%
...
```{r}
(plotme(wfbz_crit, 'WUI-Interface Percent', line = TRUE) +
  ggtitle('Community-Impacting\nDisasters')) +
  plot_spacer() +
  (plotme(wfbz_crit_all, 'WUI-Interface Percent', line = TRUE) +
    ggtitle('All Disasters') +
    ylab('')) +
  plot_layout(ncol = 1)
```
...
plot_spacer() +
  (plotme(wfbz_crit, 'WUI-Interface Percent', stack = FALSE) +
    ggtitle('Community-Impacting\nDisasters')) +
  (plotme(wfbz_crit_all, 'WUI-Interface Percent', stack = FALSE) +
    ggtitle('All Disasters') +
    ylab('')) +
  plot_layout(ncol = 4) +
  plot_spacer()
...
```{r}
(plotme(wfbz_crit, 'WUI-Intermix Percent', line = TRUE) +
  ggtitle('Community-Impacting\nDisasters')) +
  plot_spacer() +
  (plotme(wfbz_crit_all, 'WUI-Intermix Percent', line = TRUE) +
    ggtitle('All Disasters') +
    ylab('')) +
  plot_layout(ncol = 1)
```
...
plot_spacer() +
  (plotme(wfbz_crit, 'WUI-Intermix Percent', stack = FALSE) +
    ggtitle('Community-Impacting\nDisasters')) +
  (plotme(wfbz_crit_all, 'WUI-Intermix Percent', stack = FALSE) +
    ggtitle('All Disasters') +
    ylab('')) +
  plot_spacer() +
  plot_layout(ncol = 4)
...
wfbz_lab %>% select(WUI, `FEMA Declaration`) %>%
	table(useNA = 'ifany') %>%
	as_tibble() %>%
	ggplot() +
