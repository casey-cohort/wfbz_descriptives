...
by_state <- by_state %>% 
  arrange(desc(`FEMA Declaration Percent`)) %>%
  mutate(state_abb = factor(row_number(), levels = row_number(), labels = state_abb))
```

### Fema Declaration Liklihood by State

```{r}
ggplot() +
  geom_point(data = by_state, aes(x = state_abb, y = `FEMA Declaration Percent`)) + 
  scale_y_continuous(labels = ~format(round(100*.x)))
```
